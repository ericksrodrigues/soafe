  ...
  for x in range(1):
    ...
    def automatic_transformation(dataframe,weights="flename_weights"):
      ...
      for data in dataframe:
        ...
        #             countWei+=1
        i = 1
        for x in range(soms["shape"][0]):
          for j in range (soms["shape"][1]):
            distance = sqrt(sum([(data[i] - x)**2 for i,x in enumerate(weights[i])]))
          ...
          # )
        new_data.append(np.concatenate(([],[bmu_position_i,bmu_position_j,bmu_distance])))
      return new_data
    nf = []
